...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType &p)
{
    ...
    // add if loading factor need resize
    if (double(totalCount) / CAPACITIES[mIndex_] >= resizeAlpha)
    {
        resize();
    }
    ...
    else if (table_[index]->item.first == p.first)
    {
...
void HashTable<K, V, Prober, Hash, KEqual>::remove(const KeyType &key)
{
    ...
    else if (table_[index]->item.first == key)
    {
...
{
    std::vector<HashItem *> table_2;
    std::vector<HashItem *> old_table_ = table_;
    HASH_INDEX_T index;
    int mIndexOld = mIndex_;
    mIndex_++;

    // initialize the second vector to new size
    for (int i = 0; i < CAPACITIES[mIndex_]; i++)
    {
        table_2.push_back(nullptr);
    }

    table_ = table_2;

    // insert all non-deleted items into the new table
    for (int i = 0; i < CAPACITIES[mIndexOld]; i++)
    {
        if (old_table_[i] != nullptr && old_table_[i]->deleted == false)
        {
            index = this->probe(old_table_[i]->item.first);
            table_[index] = old_table_[i];
        }
        else if (old_table_[i] != nullptr && old_table_[i]->deleted == true)
        {
            delete old_table_[i];
        }
    }
